...
import type { ChatState, ProjectContext } from './types';
...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  private handleGetMessages(): Response {
    // Return a plain JSON object from the state to prevent serialization issues.
    const plainState = {
   ...
   */
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean; context?: ProjectContext }): Promise<Response> {
    const { message, model, stream, context } = body;
    // Validate input
    if (!message?.trim()) {
      return Response.json({
        success: false,
